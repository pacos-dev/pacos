...
/**
 * Put dock to overlay container
 */
window.attachDockToOverlay = function () {

    const dock = document.getElementById('dockContainer');
    if (!dock) return;

    const overlayContainer =
        document.querySelector('vaadin-overlay-container') ||
        document.body;

    if (!dock.__attachedToOverlay) {
        overlayContainer.appendChild(dock);
        dock.__attachedToOverlay = true;
    }
};

window.initDockHover = function (popover) {
    const EDGE_HEIGHT = 6;
    let visible = true;

    document.addEventListener('mousemove', (e) => {
        const atBottomEdge =
            window.innerHeight - e.clientY <= EDGE_HEIGHT;

        if (atBottomEdge && !visible) {
            popover.opened = true;
            visible = true;
        }
